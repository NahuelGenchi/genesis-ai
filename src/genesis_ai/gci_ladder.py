...
def _suite_task_hashes(path: str | Path) -> dict[str, Any]:
    ...
    generated_count = int(suite["tasks_per_domain"]) * len(suite["domains"])
    if sum(domain_counts.values()) != generated_count:
        raise ValueError(f"suite generator count drifted: {path}")
    return {
        "path": str(path),
        ...
        "duplicate_prompt_count": generated_count - len(prompt_hashes),
        "duplicate_task_count": generated_count - len(task_hashes),
        "domain_generated_counts": domain_counts,
...
def build_ladder_manifest(suite_paths: Iterable[str | Path]) -> dict[str, Any]:
    ...
    return {
        ...
        "total_generated_tasks": sum(int(item["generated_task_count"]) for item in public_entries),
        "total_unique_prompts_across_difficulties": len(global_prompt_hashes),
        "total_unique_tasks_across_difficulties": len(global_task_hashes),
